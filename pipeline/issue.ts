...
import { packAttrs, validCredentialAttrs } from './attrs.js';
...
import { evaluateIdentityPolicy, type IdentityPolicyV1 } from './identity-policy.js';
...
export interface IssueRequest {
  ...
  /** Expected policy-local grade. It is checked against performed methods, never trusted directly. */
  assurance?: number;
  identityPolicy: IdentityPolicyV1;
  /** Approved/synthetic data-flow snapshot. The web boundary requires it for new API issuance. */
  processingPolicy?: ProcessingPolicyEvidenceV1;
  /** Digest of the exact notice text whose wallet signature was verified. */
  consentStatementHash?: string;
  /** Consent-bound retention version committed with the issuance evidence. */
...
export type IssueOutcome =
  | { status: 'ISSUED'; attrs: string; claimsRoot: string; evidenceHash: string;
      ...
      claims: Claim[]; evidence: unknown; screeningSnapshotId?: string }
...
export async function runIssuance(
  ...
  if (req.processingPolicy) {
    chain.append({ step: 'processing_policy', at, payload: { ...req.processingPolicy,
      consentStatementHash: req.consentStatementHash ?? req.processingPolicy.consentStatementHash } });
  }
  if (req.retentionPolicy) {
    chain.append({ step: 'retention_policy', at, payload: { ...req.retentionPolicy } });
  }
  ...
  // Customer identity policy: keep document/account control distinct from actual-person and UBO
  // checks, derive the policy-local assurance, and fail closed before creating claims.
  const methods = adapterResult.methods | screening.methodsApplied;
  const identity = evaluateIdentityPolicy(req.identityPolicy, methods, req.kind ?? 1);
  chain.append({ step: 'identity_policy', at, payload: { ...identity } });
  if (!identity.passed) {
    const details = [...identity.missing, ...identity.violations].join(',');
    return { status: 'REJECTED', reason: `identity policy requirements not met: ${details}`,
      evidenceHash: chain.hash, evidence: chain.export() };
  }
  if (req.assurance !== undefined && req.assurance !== identity.assurance) {
    return { status: 'REJECTED', reason: `assurance does not match policy-local performed-check basis: expected ${identity.assurance}`,
      evidenceHash: chain.hash, evidence: chain.export() };
  }

  // 5. claim commitment. Salts are returned to the subject and retained only in encrypted
  // recovery/evidence storage by the API; they are never emitted on chain.
  const claims: Claim[] = [
  ...
  if (!validCredentialAttrs(attrs, issuedAt)) throw new Error('unsupported credential schema');
  ...
  return {
    ...
    claims, evidence: chain.export(), screeningSnapshotId: screening.evidence.sourceSnapshot?.snapshotId,
  };
}
