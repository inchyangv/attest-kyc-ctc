...
if want 6; then
  ...
  if [ "${RECORD:-0}" = "1" ]; then
    ...
    cast call $REG 'isVerified(address,uint256)(bool)' "$TAKE_B_SUBJECT" 2 --rpc-url "$CC3"  # -> true for a sandbox issuance
...
if want 8; then
  ...
  cast call $ASC 'tombstone(address)(bool)' $RWA_CONTROL --rpc-url "$CC3"  # -> false
  cast call $REG 'isVerified(address,uint256)(bool)' $RWA_CONTROL 1 --rpc-url "$CC3"  # -> false
  cast call $REG 'isVerified(address,uint256)(bool)' $RWA_CONTROL 2 --rpc-url "$CC3"  # -> false
